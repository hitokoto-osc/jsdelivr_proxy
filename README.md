...
* **异步填充**：首个未命中请求收到上游响应头后即可流式返回，完整下载后在后台压缩入库；下载失败不会缓存残缺正文，客户端断开后仍继续填充。
* **并发合并回源**：同一路径的后续未命中请求等待正在进行的缓存填充，完成后读取缓存。
...
命中时根据请求的 `Accept-Encoding`（包含 `q=0` 与通配符）判断客户端能力，
支持缓存编码时直接发送 zstd / Brotli 字节，并设置 `Content-Encoding` 和 `Vary: Accept-Encoding`。
不支持时才解压，交给全局响应压缩器协商 gzip、Brotli、zstd 或 deflate；
未声明编码能力时返回未压缩内容。全局压缩器也处理回源流及其他路由的响应，已编码的响应不会重复压缩。
